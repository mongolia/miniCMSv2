<?php
	...
	define( 'CPATH', ABS_DIR.'modules/grabber/csvUpload/' );
	...
	class grabLycos extends baseFunction
	{
		...
		function grabLycos( $username, $password ){
			...
			if ( !empty( $username ) )
				...
				echo( 'Lycos ийн хэрэглэгчийн нэрийг оруулна уу' );
			if ( !empty( $password ) )
				$this->_password = $password;
			else
				echo( 'Lycos хэрэглэгчийн нууц үгийг оруулна уу' );
		...
		function getContactList(){
			...
			fclose($fp);
		 	...
		 	$fileContentArr       = file(GRABBER_COOKIE_FILE);
