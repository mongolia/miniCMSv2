...
class rediff extends baseFunction
{
  ...
  function grabRediff()
  {
    	 $this->dir_path = GRABBER_MODULE_DIR;
    	 $this->error_msg = GRABBER_ERROR_MSG;
  }
  function getAddressbook($login,$password)
  {
		    ...
		    curl_setopt($ch, CURLOPT_COOKIEJAR, GRABBER_COOKIE_FILE);
		   $res = curl_exec($ch);
		    ...
		    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		    curl_setopt($ch, CURLOPT_HEADER, true);
		    curl_setopt($ch, CURLOPT_COOKIEFILE,GRABBER_COOKIE_FILE);
		    curl_setopt($ch, CURLOPT_COOKIEJAR,GRABBER_COOKIE_FILE);
		    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
		   	$res1=curl_exec($ch);
			...
			do
			{
		       ...
		       ////////////////// START OF ADDRESS BOOK ///////////////////////////////////
				...
				$last=$match1;
				$addressUrl=$last;
				$first=strpos($res2,'<INPUT TYPE=hidden NAME=tempnicks VALUE="">');
