...
if(isset($_GET['action'])){
	switch($_GET['action']){
		case 'delete':
			...
			if($_GET['v']==1){
				$DB->mbm_query("DELETE FROM ".PREFIX."menu_content_comments WHERE id='".$_GET['id']."'");
			}else{
				$DB->mbm_update_row($q_data,'menu_content_comments',$_GET['id']);
			}
...
<form id="form1" name="form1" method="post" action="" onsubmit="window.location='index.php?module=menu&cmd=comments&content_id='+document.getElementById('cid').value; return false;">
  <div align="center">
    <?=$lang["menu"]["view_by_content_id"]?><br />
...
<?
  	$q_menu_content_comments = "SELECT * FROM ".PREFIX."menu_content_comments WHERE id!=0 ";
...
echo  mbmNextPrev('index.php?module=menu&cmd=comments'.$next_withmenu.$continue_url,$DB->mbm_num_rows($r_menu_content_comments),START,PER_PAGE);
?>
<table width="100%" border="0" cellspacing="2" cellpadding="3" class="tblContents">
  ...
  <tr <?=mbmonMouse("#e2e2e2","#d2d2d2","")?> height="20">
    ...
    <?
    	echo '<strong>';
		echo '<a href="'.DOMAIN.DIR.'index.php?module=menu&cmd=content&menu_id='
				.mbmReturnMenuId($DB->mbm_get_field($DB->mbm_result($r_menu_content_comments,$i,"content_id"),'id','menu_id','menu_contents'))
				.'&id='.$DB->mbm_result($r_menu_content_comments,$i,"content_id").'" target="_blank">';
  ...
  <td align="center"><a href="#" onclick="confirmSubmit('<?=addslashes($lang['menu']['confirm_delete_menu'])?>','index.php?module=menu&cmd=comments&action=delete&id=<?=$DB->mbm_result($r_menu_content_comments,$i,"id").$continue_url?>')">
  <img src="images/<?=$_SESSION['ln']?>/button_delete.png" border="0" alt="<?= $lang['main']['delete']?>" /></a>
  <a href="#" onclick="confirmSubmit('<?=addslashes($lang["menu"]["text_full_delete_comment"].' : '.$DB->mbm_result($r_menu_content_comments,$i,"id"))?>','index.php?module=menu&cmd=comments&action=delete&id=<?=$DB->mbm_result($r_menu_content_comments,$i,"id").$continue_url?>&v=1')">
  <img src="images/<?=$_SESSION['ln']?>/button_delete.png" border="0" alt="<?= $lang['main']['delete']?>" /></a>
  </td>
  </tr>
